...
function MyJobPostings() {
  ...
  const handleEditSave = async () => {
    try {
      ...
      const updateData = {
        ...
        minSalary: editJobModal.formData.minSalary
          ? parseFloat(editJobModal.formData.minSalary)
          : null,
        maxSalary: editJobModal.formData.maxSalary
          ? parseFloat(editJobModal.formData.maxSalary)
          : null,
  ...
  const handleViewApplicants = async (job) => {
    try {
      ...
      const applicants = await applicationAPI.getJobApplicationsWithUserDetails(
        job.id
      );
      ...
      for (const applicant of applicants || []) {
        try {
          ...
          if (userId) {
            ...
            console.log(
              `🔍 JobSeeker profile for user ${userId}:`,
              userProfile
            );
          ...
          console.error(
            `❌ Failed to load JobSeeker details for applicant:`,
            error
          );
          applicantsWithFullDetails.push(applicant);
        }
      ...
      console.log(
        ...
        applicantsWithFullDetails
      );
      setApplicantsModal({
        open: true,
        job,
        applicants: applicantsWithFullDetails,
      });
    } catch (error) {
  ...
  const handleViewApplicantDetails = async (application) => {
    try {
      ...
      console.log(
        "🔍 Resume document ID:",
        userProfile?.jobSeeker?.resumeDocumentId
      );
      console.log(
        "🔍 Cover letter document ID:",
        userProfile?.jobSeeker?.coverLetterDocumentId
      );
      ...
      if (
        ...
        try {
          ...
          console.log(
            "✅ Updated profile with linked documents:",
            updatedProfile
          );
  ...
  const handleUpdateApplicationStatus = async (applicationId, newStatus) => {
    try {
      ...
      if (
        ...
        setSuccessMessage(
          "Application status updated locally (backend not available)"
        );
      }
    }
  };
  ...
  const handleScheduleInterviewWithDateTime = async (application) => {
    try {
      ...
      if (
        ...
        setSuccessMessage(
          "Interview status updated locally (backend not available)"
        );
      }
    }
  };
    ...
    try {
      switch (action) {
        ...
        case "hold":
          ...
          // Update job status in local state
          setJobs((prev) =>
            prev.map((j) => (j.id === job.id ? { ...j, status: "HOLD" } : j))
          );
  ...
  return (
    <PageContainer>
          ...
          <Grid item xs={12} sm={6} md={3}>
            <DashboardCard>
              <Box sx={{ textAlign: "center", p: 2 }}>
                ...
                >
                  {jobs.reduce(
                    (total, job) => total + (job.applications || 0),
                    0
                  )}
                </Typography>
                <Typography variant="body2" color="text.secondary">
      ...
      >
        <DialogTitle
          sx={{ pb: 2, borderBottom: "2px solid", borderColor: "primary.main" }}
        >
          <Box
            sx={{
              display: "flex",
            ...
            <Box>
              ...
              <Box sx={{ mb: 4, p: 3, bgcolor: "grey.50", borderRadius: 3 }}>
                ...
                >
                  ...
                  <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                    ...
                    <Typography variant="body1" sx={{ fontWeight: 500 }}>
                      {viewJobModal.job.jobType?.replace("_", " ") ||
                        "Full Time"}
                    </Typography>
                  </Box>
                </Box>
                {viewJobModal.job.minSalary && viewJobModal.job.maxSalary && (
                  <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                    ...
                    >
                      ${viewJobModal.job.minSalary.toLocaleString()} - $
                      {viewJobModal.job.maxSalary.toLocaleString()}
                    </Typography>
                  </Box>
                )}
              </Box>
              ...
              <Grid container spacing={4}>
                ...
                <Grid item xs={12} md={8}>
                  ...
                  {/* Required Skills */}
                  {viewJobModal.job.requiredSkills &&
                    viewJobModal.job.requiredSkills.length > 0 && (
                      <Box sx={{ mb: 4 }}>
                        <Typography
                          variant="h6"
                          sx={{
                        ...
                        <Box sx={{ display: "flex", flexWrap: "wrap", gap: 1 }}>
                          {viewJobModal.job.requiredSkills.map(
                            (skill, index) => (
                              <Chip
                                key={index}
                                label={skill}
                ...
                <Grid item xs={12} md={4}>
                  <Box sx={{ position: "sticky", top: 20 }}>
                    <Box
                      sx={{ p: 3, bgcolor: "grey.50", borderRadius: 3, mb: 3 }}
                    >
                      <Typography
                        variant="h6"
      ...
      >
              ...
              <Grid container spacing={3}>
                ...
                <Grid item xs={12}>
                  <TextField
                    ...
                    onChange={(e) =>
                      handleEditFormChange("jobTitle", e.target.value)
                    }
                    required
                  />
                </Grid>
                ...
                <Grid item xs={12} sm={6}>
                  <TextField
                    ...
                    onChange={(e) =>
                      handleEditFormChange("minSalary", e.target.value)
                    }
                    placeholder="50000"
                  />
                </Grid>
                ...
                <Grid item xs={12} sm={6}>
                  <TextField
                    ...
                    onChange={(e) =>
                      handleEditFormChange("maxSalary", e.target.value)
                    }
                    placeholder="80000"
                  />
                </Grid>
                      ...
                      <Chip
                        ...
                        onDelete={() => {
                          const newRequirements =
                            editJobModal.formData.requirements.filter(
                              (_, i) => i !== index
                            );
                    ...
                    <TextField
                      ...
                      onKeyPress={(e) => {
                        if (e.key === "Enter") {
                          ...
                          if (
                            newSkill &&
                            !editJobModal.formData.requirements?.includes(
                              newSkill
                            )
                          ) {
                            const newRequirements = [
                              ...(editJobModal.formData.requirements || []),
                              newSkill,
                            ];
        ...
        <DialogActions>
          ...
          <Button
            ...
            disabled={
              editLoading ||
              !editJobModal.formData?.jobTitle ||
              !editJobModal.formData?.description
            }
          >
            ...
            <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
              ...
              <Button
                ...
                onClick={() => {
                  ...
                  alert(
                    `Debug: ${applicantsModal.applicants.length} applicants loaded. Check console for details.`
                  );
                }}
              >
                Debug
                ...
                <Grid item xs={12} key={application.id || index}>
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          ...
                          >
                            {application.jobSeekerProfile?.firstName?.charAt(
                              0
                            ) ||
                    ...
                    {(application.jobSeekerProfile?.skills ||
                      application.jobSeekerProfile?.certifications ||
                      ...
                      >
                        <Grid container spacing={2}>
                          ...
                          {application.jobSeekerProfile?.certifications &&
                            application.jobSeekerProfile.certifications.length >
                              0 && (
                              <Grid item xs={12} sm={6}>
                                <Typography
                                  variant="subtitle2"
      ...
      >
        <DialogTitle sx={{ pb: 1 }}>
          ...
          <Typography variant="body2" color="text.secondary">
            Schedule interview for {interviewModal.application?.user?.firstName}{" "}
            {interviewModal.application?.user?.lastName}
          </Typography>
        </DialogTitle>
        <DialogContent>
          {interviewModal.interviewData && (
              ...
              <Grid container spacing={3}>
                ...
                <Grid item xs={12}>
                  <TextField
                    ...
                    onChange={(e) =>
                      handleInterviewFormChange("location", e.target.value)
                    }
                    placeholder={
                      interviewModal.interviewData.interviewType === "IN_PERSON"
                        ? "Office address"
                ...
                <Grid item xs={12}>
                  <TextField
                    ...
                    onChange={(e) =>
                      handleInterviewFormChange("notes", e.target.value)
                    }
                    multiline
                    rows={3}
        ...
        <DialogActions>
          ...
          <Button
            ...
            disabled={
              interviewLoading ||
              !interviewModal.interviewData?.scheduledDateTime ||
              !interviewModal.interviewData?.location
            }
          >
...
function ListNewVacancy() {
  ...
  const addRequirement = () => {
    if (
      ...
      !form.requirements.includes(requirementInput.trim())
    ) {
      setForm((prev) => ({
        ...prev,
  ...
  const addQualification = () => {
    if (
      ...
      !form.qualifications.includes(qualificationInput.trim())
    ) {
      setForm((prev) => ({
        ...prev,
  ...
  const removeQualification = (qualificationToRemove) => {
    setForm((prev) => ({
      ...
      qualifications: prev.qualifications.filter(
        (qual) => qual !== qualificationToRemove
      ),
    }));
  };
...
function ViewApplicants() {
    ...
    const loadApplications = async () => {
      try {
        ...
        for (const job of jobs) {
          try {
            console.log(
              `🔍 Loading applications for job: ${job.title} (ID: ${job.id})`
            );
  ...
  const filteredApplications = applications.filter((app) => {
    ...
    if (filters.status !== "All Statuses" && status !== filters.status)
      return false;
  ...
  const downloadDocumentDirect = async (userId, documentType) => {
    try {
      ...
      const documentUrlResponse = await userAPI.getDocumentUrl(
        userId,
        documentType
      );
      ...
      if (documentUrlResponse.available && documentUrlResponse.directUrl) {
        ...
        link.download =
          documentUrlResponse.fileName || `${documentType}_${userId}.pdf`;
        link.target = "_blank";
  ...
  const loadDocumentInfo = async (userId) => {
    try {
      ...
      console.log(
        `🔍 Loading document info and availability for user: ${userId}`
      );

      // Check document availability from program_documents table
      const availabilityResponse = await userAPI.checkDocumentAvailability(
        userId
      );
      ...
      setDocumentAvailability({
        ...
        coverLetterAvailable:
          availabilityResponse.coverLetterAvailable || false,
        loading: false,
        error: null,
      });
      ...
      const [resumeInfo, coverLetterInfo] = await Promise.allSettled([
        availabilityResponse.resumeAvailable
          ? userAPI.getDocumentInfo(userId, "resume")
          : Promise.resolve(null),
        availabilityResponse.coverLetterAvailable
          ? userAPI.getDocumentInfo(userId, "coverLetter")
          : Promise.resolve(null),
      ]);
      ...
      const newDocumentInfo = {
        ...
        coverLetter:
          coverLetterInfo.status === "fulfilled" ? coverLetterInfo.value : null,
        loading: false,
        error: null,
      };

      setDocumentInfo(newDocumentInfo);
      console.log(`✅ Document info and availability loaded:`, {
  ...
  const handleRejectionWithReason = async () => {
    ...
    try {
      await handleUpdateApplicationStatus(
        ...
        "REJECTED",
        rejectionModal.rejectReason.trim()
      );
  ...
  const handleScheduleInterviewWithDateTime = async (application) => {
    try {
      const interviewData = {
        jobPostingId:
          application.jobPostingId || application.application?.jobPostingId,
        scheduledDateTime: application.interviewDateTime,
        location: "To be confirmed",
        interviewType: "IN_PERSON",
        notes: "Interview scheduled via status update",
  ...
  const handleViewProfile = async (applicant) => {
    try {
      ...
      console.log(
        ...
        userProfile?.coverLetterDocument
      );
  ...
  const handleDownloadResume = async (userId) => {
    try {
      ...
      console.log(
        "📄 Applicant profile:",
        applicantProfile,
        applicantProfile.resumeDocument
      );
      ...
      console.log(
        "📄 Resume document ID:",
        applicantProfile.resumeDocument?.fileId
      );
      ...
      if (error.message.includes("404")) {
        alert(
          "Resume document not found. The applicant may not have uploaded a resume."
        );
      } else {
  ...
  const handleDownloadCoverLetter = async (userId) => {
    try {
      ...
      console.log(
        "📄 Cover letter document ID:",
        applicantProfile.coverLetterDocumentId
      );
      ...
      if (error.message.includes("404")) {
        alert(
          "Cover letter document not found. The applicant may not have uploaded a cover letter."
        );
      } else {
    ...
    try {
      ...
      if (documentUrlResponse.available && documentUrlResponse.directUrl) {
        ...
        if (newTab) {
          ...
          console.log(
            `📄 File details: ${documentUrlResponse.fileName} (${documentUrlResponse.fileSize} bytes)`
          );
        } else {
          ...
          document.body.removeChild(link);
          console.log(
            `📄 Document downloaded as: ${documentUrlResponse.fileName}`
          );
        }
        return;
      }
      ...
      console.log(
        `🔍 Direct URL not available, falling back to blob download...`
      );
      ...
      if (newTab) {
        ...
        console.log(
          `📄 File details: ${documentInfo.fileName} (${documentInfo.fileSize} bytes)`
        );
      } else {
        console.warn(`⚠️ Popup blocked, trying to download instead`);
        // Fallback to download if popup is blocked
        ...
        link.href = url;
        link.download =
          documentInfo.fileName || `${documentType}_${userId}.pdf`;
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
  ...
  return (
    <PageContainer>
          ...
          <AnimatedBox animation="fadeInUp" delay={0.4}>
            <DashboardCard sx={{ p: 2 }}>
              ...
              >
                ...
                {Array.from(
                  new Set(
                    applications.map((app) => app.jobTitle || "Unknown Job")
                  )
                ).map((jobTitle) => (
                    ...
                    <Grid container spacing={2} sx={{ mb: 2 }}>
                      ...
                      <Grid item xs={12} sm={6}>
                        ...
                        {(application.application?.status === "REJECTED" ||
                          ...
                          (application.application?.rejectReason ||
                      ...
                      >
                        ...
                        >
                          ...
                          <Button
                            ...
                            variant="contained"
                            onClick={() =>
                              handleScheduleInterviewWithDateTime(application)
                            }
            ...
            {profileModal.userProfile && (
              <Box sx={{ display: "flex", gap: 1 }}>
                <Button
                  ...
                  onClick={() => {
                    const userId =
                      profileModal.applicant?.application?.userId ||
                      profileModal.applicant?.user?.id;
            ...
            <Box sx={{ pt: 1 }}>
              <Grid container spacing={3}>
                ...
                <Grid item xs={12}>
                  ...
                  >
                    <Typography variant="h4" sx={{ fontWeight: 700, mb: 1 }}>
                      {profileModal.userProfile?.jobSeeker?.firstName}{" "}
                      {profileModal.userProfile?.jobSeeker?.lastName}
                    </Typography>
                ...
                <Grid item xs={12} sm={6}>
                  ...
                  <Typography variant="body1" sx={{ mb: 2, fontWeight: 500 }}>
                    {profileModal.userProfile?.jobSeeker?.phoneNumber ||
                      profileModal.applicant.phoneNumber ||
                      "N/A"}
                  </Typography>
                </Grid>
                ...
                <Grid item xs={12}>
                  ...
                  <Typography variant="body1" sx={{ mb: 2, fontWeight: 500 }}>
                    {profileModal.userProfile?.jobSeeker?.address ||
                      profileModal.applicant.address ||
                      "N/A"}
                  </Typography>
                </Grid>
                ...
                {profileModal.userProfile?.jobSeeker?.skills &&
                  profileModal.userProfile.jobSeeker.skills.length > 0 && (
                    <Grid item xs={12}>
                      <Typography variant="subtitle2" color="text.secondary">
                        Skills
                      </Typography>
                      ...
                      >
                        {profileModal.userProfile.jobSeeker.skills.map(
                          (skill, index) => (
                            <Chip
                              key={index}
                              label={skill}
                ...
                {profileModal.userProfile?.jobSeeker?.certifications &&
                  profileModal.userProfile.jobSeeker.certifications.length >
                    0 && (
                    <Grid item xs={12}>
                      <Typography variant="subtitle2" color="text.secondary">
                        Certifications
                      ...
                      >
                        {profileModal.userProfile.jobSeeker.certifications.map(
                          (cert, index) => (
                            <Chip
                              key={index}
                              label={cert}
                ...
                <Grid item xs={12} sm={6}>
                  ...
                  <Typography variant="body1" sx={{ mb: 2 }}>
                    {profileModal.userProfile?.jobSeeker?.veteran
                      ? "Yes"
                      : "No"}
                  </Typography>
                </Grid>
                ...
                {profileModal.userProfile?.jobSeeker && (
                  <>
                    ...
                    <Grid item xs={12} sm={6}>
                      ...
                      <Typography variant="body1" sx={{ mb: 2 }}>
                        {profileModal.userProfile.jobSeeker.preferredJobType ||
                          "N/A"}
                      </Typography>
                    </Grid>
                    ...
                    <Grid item xs={12} sm={6}>
                      ...
                      <Typography variant="body1" sx={{ mb: 2 }}>
                        {profileModal.userProfile.jobSeeker.preferredLocation ||
                          "N/A"}
                      </Typography>
                    </Grid>
                    ...
                    <Grid item xs={12} sm={6}>
                      ...
                      <Typography variant="body1" sx={{ mb: 2 }}>
                        {profileModal.userProfile.jobSeeker.availability ||
                          "N/A"}
                      </Typography>
                    </Grid>
                    ...
                    <Grid item xs={12} sm={6}>
                      ...
                      <Typography variant="body1" sx={{ mb: 2 }}>
                        {profileModal.userProfile.jobSeeker.willingToRelocate
                          ? "Yes"
                          : "No"}
                      </Typography>
                    </Grid>
                  ...
                  >
                      ...
                      <Typography variant="body2" color="text.secondary">
                        {documentAvailability.resumeAvailable
                          ? documentInfo.resume?.fileName
                            ? `${documentInfo.resume.fileName} (${Math.round(
                                documentInfo.resume.fileSize / 1024
                              )} KB)`
                    ...
                    {documentAvailability.resumeAvailable && (
                      <Button
                        ...
                        onClick={() => {
                          const userId =
                            applicantsModal.selectedApplicant?.application
                              ?.userId ||
                            applicantsModal.selectedApplicant?.user?.id;
                          if (userId) handleDownloadResume(userId);
                        }}
                      >
                        Download
                        ...
                        {documentAvailability.coverLetterAvailable
                          ? documentInfo.coverLetter?.fileName
                            ? `${
                                ...
                                documentInfo.coverLetter.fileSize / 1024
                              )} KB)`
                            : "Available for download"
                          : "Not uploaded"}
                      </Typography>
                      ...
                      <Button
                        ...
                        onClick={() => {
                          const userId =
                            ...
                            applicantsModal.selectedApplicant?.user?.id;
                          if (userId)
                            downloadDocumentDirect(userId, "coverLetter");
                        }}
                      >
                        Download
                      </Button>
                      ...
                      <Box sx={{ display: "flex", gap: 2, flexWrap: "wrap" }}>
                          ...
                          <Button
                            ...
                            onClick={() => {
                              const userId =
                                ...
                                applicantsModal.selectedApplicant?.user?.id;
                              if (userId)
                                openDocumentInNewTab(userId, "resume");
                            }}
                          >
                            Open Resume
                          </Button>
                          ...
                          <Button
                            ...
                            onClick={() => {
                              const userId =
                                ...
                                applicantsModal.selectedApplicant?.user?.id;
                              if (userId)
                                openDocumentInNewTab(userId, "coverLetter");
                            }}
                          >
                            Open Cover Letter
                          </Button>
                          ...
                          onClick={async () => {
                            ...
                            if (userId) {
                              try {
                                ...
                                alert(
                                  "Documents linked successfully! Please try opening the documents now."
                                );
                              } catch (error) {
                                console.error(
                        ...
                        <Button
                          ...
                          onClick={() => {
                            ...
                            if (userId) {
                              console.log(
                                "🔍 Testing document access for user:",
                                userId
                              );
                              console.log(
                                "🔍 Profile data:",
                              ...
                              alert(
                                `Testing documents for user: ${userId}\nCheck console for details.`
                              );
                            }
                          }}
                        >
                        ...
                        <Button
                          ...
                          onClick={async () => {
                            ...
                            if (userId) {
                              try {
                                console.log(
                                  "🔍 Debugging file structure for user:",
                                  userId
                                );
                                const fileStructure =
                                  await userAPI.debugFileStructure(userId);
                                console.log(
                                  "📄 File structure debug info:",
                                  fileStructure
                                );
                                alert(
                                  `File structure debug info for user: ${userId}\nCheck console for details.`
                                );
                              } catch (error) {
                                console.error(
                                  "❌ Failed to debug file structure:",
                                  error
                                );
                                alert(
                                  "Failed to debug file structure: " +
              ...
              <Grid container spacing={3}>
                <Grid item xs={12}>
                  <Typography
                    ...
                    sx={{ fontWeight: 600, mb: 2, color: "primary.main" }}
                  >
                    {contactModal.userProfile?.jobSeeker?.firstName}{" "}
                    {contactModal.userProfile?.jobSeeker?.lastName}
                  </Typography>
                </Grid>
                ...
                <Grid item xs={12}>
                  ...
                  <Typography variant="body1" sx={{ mb: 2, fontWeight: 500 }}>
                    {contactModal.userProfile?.jobSeeker?.email ||
                      contactModal.applicant.email ||
                      "N/A"}
                  </Typography>
                </Grid>
                ...
                <Grid item xs={12}>
                  ...
                  <Typography variant="body1" sx={{ mb: 2, fontWeight: 500 }}>
                    {contactModal.userProfile?.jobSeeker?.phoneNumber ||
                      contactModal.applicant.phoneNumber ||
                      "N/A"}
                  </Typography>
                </Grid>
                ...
                <Grid item xs={12}>
                  ...
                  <Typography variant="body1" sx={{ mb: 2, fontWeight: 500 }}>
                    {contactModal.userProfile?.jobSeeker?.address ||
                      contactModal.applicant.address ||
                      "N/A"}
                  </Typography>
                </Grid>
      ...
      <Dialog
        ...
        onClose={() =>
          setApplicantsModal((prev) => ({ ...prev, selectedApplicant: null }))
        }
        maxWidth="lg"
        fullWidth
      >
        ...
        >
          ...
          >
            <Box>
              ...
              <Typography variant="body2" color="text.secondary">
                {
                  applicantsModal.selectedApplicant?.profile?.jobSeeker
                    ?.firstName
                }{" "}
                {
                  applicantsModal.selectedApplicant?.profile?.jobSeeker
        ...
        <DialogContent sx={{ pt: 3 }}>
          {applicantsModal.selectedApplicant &&
          applicantsModal.selectedApplicant.profile ? (
            <Box>
              <Grid container spacing={3}>
                {/* Header Section */}
                <Grid item xs={12}>
                  ...
                  >
                    <Typography variant="h4" sx={{ fontWeight: 700, mb: 1 }}>
                      {applicantsModal.selectedApplicant?.profile?.jobSeeker
                        ?.firstName ||
                        applicantsModal.selectedApplicant?.profile?.user
                          ?.firstName}{" "}
                      {applicantsModal.selectedApplicant?.profile?.jobSeeker
                        ?.lastName ||
                        applicantsModal.selectedApplicant?.profile?.user
                    ...
                    <Typography variant="h6" sx={{ opacity: 0.9 }}>
                      {applicantsModal.selectedApplicant?.profile?.jobSeeker
                        ?.email ||
                        applicantsModal.selectedApplicant?.profile?.user?.email}
                    </Typography>
                    {(applicantsModal.selectedApplicant?.profile?.jobSeeker
                      ?.profileCompletionPercent ||
                      applicantsModal.selectedApplicant?.profile?.user
                      ...
                      <Box sx={{ mt: 2 }}>
                        <Typography variant="body2" sx={{ mb: 1 }}>
                          ...
                          {Math.round(
                            applicantsModal.selectedApplicant?.profile
                              ?.jobSeeker?.profileCompletionPercent ||
                              applicantsModal.selectedApplicant?.profile?.user
                                ?.profileCompletionPercent ||
                              0
                          )}
                ...
                {applicantsModal.selectedApplicant?.profile?.jobSeeker
                  ?.skills &&
                  applicantsModal.selectedApplicant?.profile?.jobSeeker?.skills
                    .length > 0 && (
                    <Grid item xs={12}>
                      <Typography variant="subtitle2" color="text.secondary">
                        Skills
                      ...
                      >
                        {applicantsModal.selectedApplicant?.profile?.jobSeeker?.skills.map(
                          (skill, index) => (
                            <Chip
                              key={index}
                              label={skill}
                ...
                {applicantsModal.selectedApplicant?.profile?.jobSeeker
                  ...
                  applicantsModal.selectedApplicant?.profile?.jobSeeker
                    ...
                    <Grid item xs={12}>
                      ...
                      >
                        {applicantsModal.selectedApplicant?.profile?.jobSeeker?.certifications.map(
                          (cert, index) => (
                            <Chip
                              key={index}
                              label={cert}
                ...
                <Grid item xs={12} sm={6}>
                  ...
                  <Typography variant="body1" sx={{ mb: 2 }}>
                    {applicantsModal.selectedApplicant?.profile?.jobSeeker
                      ?.expectedSalary
                      ? `$${applicantsModal.selectedApplicant?.profile?.jobSeeker?.expectedSalary.toLocaleString()}`
                      : "N/A"}
                  </Typography>
                </Grid>
                    ...
                    <Box sx={{ flex: 1 }}>
                      ...
                      <Typography variant="body2" color="text.secondary">
                        {documentInfo.coverLetter?.exists
                          ? `${documentInfo.coverLetter.fileName} (${Math.round(
                              documentInfo.coverLetter.fileSize / 1024
                            )} KB)`
                          : "Not uploaded"}
                      </Typography>
      ...
      try {
        ...
        if (response) {
          ...
          setFormData({
            ...
            streetAddress:
              response.streetAddress || response.address?.street || "",
        ...
        if (response) {
          ...
          setFormData({
            ...
            phone:
              ...
              "",
            jobPosition:
              response.jobPosition || response.position || response.title || "",
            department: response.department || "",
            profilePicture:
              response.profilePicture || response.user?.profilePicture || null,
          });
        } else {
          ...
          try {
            ...
            if (userResponse) {
              ...
              setFormData({
                ...
                jobPosition:
                  userResponse.jobPosition || userResponse.position || "",
              ...
              <Grid container spacing={3}>
                  ...
                  <Avatar
                    src={
                      formData.profilePicture
                        ? employerAPI.getProfilePictureUrl()
                        : undefined
                    }
                ...
                <Grid item xs={12} sm={6}>
                  <TextField
                    ...
                    disabled={!editMode}
                    placeholder={
                      editMode ? "Enter your full name" : "No name provided"
                    }
                    sx={{ mb: 2 }}
                  />
                </Grid>
                ...
                <Grid item xs={12} sm={6}>
                  <TextField
                    ...
                    disabled={!editMode}
                    placeholder={
                      editMode ? "Enter your phone number" : "No phone provided"
                    }
                    sx={{ mb: 2 }}
                  />
                </Grid>
